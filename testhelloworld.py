...
class TestHemsida(TestCase):
    ...
    def testPageText(self):
        self.browser.get(path.join(path.dirname(getcwd()), 'index.html'))
        self.assertIn("Viggo", self.browser.page_source)
        self.assertIn("Sörensen", self.browser.page_source)
        self.assertIn("Personlig", self.browser.page_source)
